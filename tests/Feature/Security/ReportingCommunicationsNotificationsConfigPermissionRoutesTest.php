...
final class ReportingCommunicationsNotificationsConfigPermissionRoutesTest extends TestCase
{
    private const EXPECTED_ROUTE_COUNT = 123;
    ...
    private const EXPECTED_OPERATION_COUNTS = [
        'mutation' => 61,
        'read' => 62,
    ];

    private const EXPECTED_CRITICAL_DOWNLOADS = [
    ...
    public function test_manifest_scope_is_complete_unique_and_reconciled(): void
    {
        ...
        $this->assertCount(
            ...
            $routes,
            'O manifesto da Sprint 47H deve conter exatamente 123 rotas.',
        );
        ...
        foreach ($routes as $index => $route) {
            ...
            $operationType = $this->requiredString(
                $route,
                'operation_type',
                $routeName,
            );
            $permissionOrigin = $this->requiredString(
                $route,
                'permission_origin',
                $routeName,
            );
            ...
            $this->assertTrue(
                ...
                "{$routeName}: todas as rotas da Sprint 47H devem exigir MFA.",
            );
            ...
            $this->assertArrayHasKey(
                $permissionOrigin,
                $permissionsByOrigin,
                "{$routeName}: origem de permission inválida.",
            );
            $permissionsByOrigin[$permissionOrigin][] = $permission;

            $critical = ($route['critical_download_or_export'] ?? null)
                === true;

            if ($critical) {
                $criticalDownloads[] = $routeName;
            }

            if ($operationType === 'mutation' || $critical) {
                $this->assertTrue(
                    ($route['audit_required'] ?? null) === true,
                    "{$routeName}: operação crítica/mutável exige audit.",
                );
            }
        }
        ...
        $this->assertSame(
            ...
            $domainCounts,
            'A distribuição dos bounded contexts da Sprint 47H foi alterada.',
        );

        ksort($operationCounts);

        $this->assertSame(
            self::EXPECTED_OPERATION_COUNTS,
            $operationCounts,
            'A classificação read/mutation do manifesto foi alterada.',
        );

        sort($criticalDownloads);
        $expectedCriticalDownloads = self::EXPECTED_CRITICAL_DOWNLOADS;
        sort($expectedCriticalDownloads);

        $this->assertSame(
            $expectedCriticalDownloads,
            $criticalDownloads,
            'Os downloads críticos do manifesto foram alterados.',
        );

        $this->assertCount(
            29,
            array_unique($permissionsByOrigin['existing']),
            'O manifesto deve reutilizar 29 permissions existentes.',
        );
        $this->assertCount(
            57,
            array_unique($permissionsByOrigin['new']),
            'O manifesto deve introduzir 57 permissions semânticas.',
        );
        $this->assertCount(
            86,
            array_unique([
                ...$permissionsByOrigin['existing'],
                ...$permissionsByOrigin['new'],
            ]),
            'O manifesto deve resolver 86 permissions únicas.',
        );
    }

    public function test_backoffice_has_no_fixed_role_middleware_remaining(): void
    {
        ...
        foreach ($routeCollection->getRoutes() as $route) {
            ...
            if (
                ! is_string($name)
                || ! str_starts_with($name, 'backoffice.')
            ) {
                continue;
            }
        ...
        $this->assertSame(
            [],
            $fixedRoleRoutes,
            'O backoffice não pode conservar middleware role:* fixo.',
        );
    }
    ...
    public function test_runtime_routes_match_permission_first_manifest(): void
    {
        ...
        $this->assertSame(
            self::EXPECTED_ROUTE_COUNT,
            $validatedRoutes,
            'O teste não validou todas as 123 rotas da Sprint 47H.',
        );
    }
    ...
    private function manifestRoutes(): array
    {
        ...
        if ($json === false) {
            self::fail(
                "Não foi possível ler o manifesto da Sprint 47H: {$path}",
            );
        }
        ...
        if (! is_array($decoded)) {
            self::fail(
                'O manifesto da Sprint 47H não contém um objeto JSON válido.',
            );
        }
        ...
        if (! is_array($rawRoutes)) {
            self::fail(
                'O manifesto da Sprint 47H não contém a coleção routes.',
            );
        }
