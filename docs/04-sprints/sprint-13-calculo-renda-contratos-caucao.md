...
A Sprint 13 deve ser executada depois da Sprint 12, usando apenas atribuições aceites e marcadas como prontas para contrato.
...
Sprint 13 — Cálculo de Renda, Contratos e Caução
Sprint 14 — Gestão Contratual e Pós-Atribuição
```
...
Implementar o módulo contratual da plataforma.

A Sprint 13 deve transformar uma atribuição aceite numa proposta contratual formal, incluindo cálculo de renda, caução, minuta contratual parametrizável, geração de contrato e gestão de estados contratuais.
...
Esta sprint não deve implementar cobrança real, faturação, recibos, assinatura digital externa ou gestão pós-contratual avançada.

---

# 2. Instrução operacional para Codex

Executa apenas esta Sprint 13.
...
input_value deve evitar dados pessoais excessivos.
```

---

## 9.5 RentManualReview
...
Não incluir dados reais em seeders.
...
## 9.16 LeaseContractStatusHistory
...
## 12.7 ContractPlaceholderService
...
## 12.10 LeaseContractNumberService
...
## 12.11 LeaseContractDocumentService
...
## 12.13 LeaseContractValidationService
...
## 12.14 LeaseContractSignatureService
...
## 12.15 LeaseContractStatusService
...
## 12.17 ContractActivationService
...
## 12.18 ContractNotificationService
...
### LeaseContractDocumentController
...
A área de minutas deve permitir:

```text
Criar minuta
Editar minuta em rascunho
Ativar minuta
Arquivar minuta
Duplicar minuta
Associar cláusulas
Ver placeholders disponíveis
Pré-visualizar contrato com dados fictícios
```
...
A página de caução deve mostrar:
...
# 19. UX obrigatória para candidato
...
# 23. Integração com atribuição — Sprint 12
...
# 25. Integração com notificações
...
A Sprint 13 só está concluída quando a plataforma permitir transformar uma atribuição aceite num contrato formal, com renda calculada, caução registada, minuta contratual parametrizável, documento contratual gerado, validação interna e ativação contratual com atualização do estado da habitação.

O resultado deve permitir que a Sprint 14 trate a gestão contratual e pós-atribuição a partir de contratos ativos.

---

# 35. Estado de execução em 12/06/2026

## Implementado

- Motor de cálculo de renda configurável por programa/concurso.
- Métodos de cálculo por taxa de esforço, valor fixo, escalão de rendimento e revisão manual.
- Aplicação de renda mínima, renda máxima, taxa de esforço e caução por meses de renda.
- Snapshot e detalhes do cálculo.
- Revisão manual com justificação, aprovação/rejeição e auditoria.
- Minutas e cláusulas contratuais parametrizáveis.
- Evolução compatível do model/tabela `contracts` existente para contrato processual.
- Criação de contrato apenas a partir de atribuição aceite/pronta para contrato e cálculo aprovado.
- Partes, cláusulas snapshot, caução, histórico de estado e notificações internas.
- Geração de documento contratual HTML em storage privado.
- Download autorizado por policy/controller.
- Emissão, validação interna, assinatura/registo manual, caução paga/dispensada e ativação.
- Ativação com atualização do estado da habitação.
- Área candidata para consulta de contratos, documentos e caução próprios.
- Backoffice para regras, cálculos, minutas, cláusulas, contratos e cauções.

## Ficheiros criados

- Migration: `database/migrations/2026_06_12_020000_create_contractual_tables.php`.
- Enums contratuais/renda/caução em `app/Enums`.
- Models: `RentRuleSet`, `RentRule`, `RentCalculation`, `RentCalculationDetail`, `RentManualReview`, `ContractTemplate`, `ContractClause`, `ContractTemplateClause`, `LeaseContractParty`, `LeaseContractClause`, `ContractDeposit`, `LeaseContractDocument`, `LeaseContractValidation`, `LeaseContractSignature`, `LeaseContractStatusHistory`.
- Controllers backoffice e candidate para contratos, cálculos, minutas, cláusulas, documentos, validações, assinaturas e caução.
- Form Requests para regras, cálculos, revisão manual, contratos, documentos, validações, assinaturas e caução.
- Policies específicas do módulo contratual.
- Services em `app/Services/Contracts`.
- Views Blade em `resources/views/backoffice/contracts`, `resources/views/candidate/contracts` e `resources/views/contracts/documents`.
- Factories e seeders opcionais para regras, minutas, cláusulas e entidades contratuais.
- Teste `tests/Feature/Sprint13ContractsRentDepositTest.php`.

## Ficheiros alterados

- `app/Models/Contract.php` e relações em `Program`, `Contest`, `Application`, `Allocation`, `HousingUnit`, `Household` e `User`.
- `app/Http/Controllers/ContractController.php` para compatibilidade do CRUD legado com campos protegidos.
- `app/Policies/ContractPolicy.php`.
- `app/Enums/ContractStatus.php` e `app/Enums/OfficialNotificationType.php`.
- `routes/web.php`.
- `resources/views/layouts/navigation.blade.php`.
- Documentação em `docs/architecture`, `docs/product`, `docs/security`, `docs/backlog` e `docs/qa`.

## Comandos executados até este registo

- `php artisan route:list`: sucesso, 506 rotas.
- `php artisan migrate`: sucesso, migration contratual aplicada.
- `php artisan test --filter=Sprint13ContractsRentDepositTest`: sucesso, 4 testes/49 asserções.
- `php artisan test`: sucesso, 122 testes/721 asserções.

## Problemas encontrados e correções

- Campos críticos protegidos por `guarded` não eram persistidos quando alguns services usavam `create()`. Corrigido com `forceFill()` dentro dos services transacionais, mantendo proteção contra mass assignment externo.
- Ativação contratual exige confirmação explícita no request; teste ajustado para passar pela validação e testar a regra de domínio real.
- A geração PDF real não existe porque o projeto não tem biblioteca/infraestrutura PDF instalada. A sprint gera HTML privado e deixa ponto de integração.

## Validações jurídicas pendentes

- Fórmulas reais de renda, mínimos, máximos e taxa de esforço.
- Limites e exigibilidade da caução.
- Minutas, cláusulas, prazos, condições especiais e textos de comunicação.
- Regras de retenção e exportação de contratos, cálculos e documentos contratuais.
- Prova externa de envio e assinatura digital, quando aplicável.

## Fora de âmbito confirmado

- Não foi implementada cobrança real.
- Não foi implementada faturação, recibos ou reconciliação financeira.
- Não foi implementada assinatura digital externa.
- Não foi implementada gestão pós-contratual avançada.
- Não foram implementados pagamentos, incumprimentos, revisão de renda, manutenção ou vistorias.
