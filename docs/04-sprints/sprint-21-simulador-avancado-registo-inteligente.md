...
A Sprint 21 deve evoluir o percurso inicial do cidadão, permitindo que este avalie previamente a sua situação, compreenda impedimentos, identifique concursos potencialmente elegíveis, simule tipologia adequada, estime renda mensal e reutilize dados no registo e na candidatura.

Esta sprint deve respeitar a regra funcional central da plataforma:

```text
O registo deve anteceder qualquer candidatura.
O simulador condiciona o acesso à submissão.
O candidato acompanha o estado do processo.
Os dados devem poder ser reutilizados em futuras candidaturas.
```

A simulação é indicativa e não substitui a análise formal do Município.

---

# 1. Objetivo da Sprint

Melhorar o processo de adesão antes da candidatura através de:
...
A plataforma deve permitir que o cidadão:

```text
Simule a sua elegibilidade antes de submeter candidatura
Compreenda critérios favoráveis e impedimentos
Saiba qual a tipologia habitacional potencialmente adequada
Veja estimativa indicativa de renda mensal
Receba recomendações de concursos compatíveis
Guarde dados da simulação no seu registo
Reutilize dados na candidatura
Atualize dados antigos sem repetir todo o processo
Renove o registo de forma simplificada
```

A plataforma deve permitir que o Município:

```text
Configure regras de simulação
Configure mensagens explicativas
Consulte estatísticas agregadas de simulações
Identifique impedimentos frequentes
Reduza candidaturas manifestamente incompletas ou inelegíveis
Melhore a qualidade dos dados recebidos
Preserve trilho de auditoria e RGPD
```
...
Executa apenas esta Sprint 21.

Não avances para Sprint 22 ou qualquer sprint futura sem validação explícita.
...
O projeto tem atualmente **2471 erros PHPStan legados**.
...
## 3.3 Verificação PHPStan antes de publicar
...
## 9.3 Pré-preenchimento da candidatura
...
## 9.4 Renovação simplificada do registo
...
## 11.5 SimulationRecommendedContest
...
## 11.6 CandidateDataReuseProfile
...
## 13.1 AdvancedEligibilitySimulatorService
...
## 13.4 SimulationDataCompletenessService
...
## 13.5 SimulationImpedimentDetector
...
## 13.6 TypologyRecommendationService
...
## 13.8 ContestRecommendationService
...
## 13.9 ApplicationPrefillService
...
## 13.10 CandidateDataReuseService
...
## 13.11 RegistrationRenewalService
...
## Public\AdvancedSimulatorController
...
## Candidate\SimulationController
...
## Candidate\ApplicationPrefillController
...
## Candidate\RegistrationRenewalController
...
## Backoffice\SimulatorInsightController
...
Adaptar à estrutura real de rotas.

---

# 19. Rotas de backoffice
...
# 23. Regras de cálculo e simulação
...
# 24. Persistência e reutilização de dados
...
# 26. Backoffice — insights do simulador
...
## 28.6 Testes de concursos recomendados
...
## 28.7 Testes de pré-preenchimento
...
# 29. PHPStan específico da Sprint 21
...
# 30. Comandos obrigatórios finais
...
# 35. Estado de execução — 2026-06-18

## Implementado

- Simulador público anónimo em `/simulador`.
- Simulador autenticado em `/area-candidato/simulacoes`.
- Persistência em `simulation_sessions`, `simulation_input_snapshots`, `simulation_results`, `simulation_impediments` e `simulation_recommended_contests`.
- Recomendação indicativa de tipologia.
- Estimativa indicativa de renda.
- Deteção de impedimentos e avisos.
- Recomendação de concursos publicados.
- Pré-preenchimento controlado de candidatura.
- Perfil de reutilização de dados do candidato.
- Renovação simplificada do Registo de Adesão.
- Backoffice de insights e configuração.
- Policies, Form Requests, services, factories, seeder e testes.
...
- Validar juridicamente os textos explicativos e limites do simulador.
- Validar com DPO a retenção dos snapshots e métricas agregadas.
- Evoluir futuramente para dry-run explícito dos motores formais, caso seja exposto por serviço dedicado.
- Continuar remediação PHPStan global sem reduzir nível, criar baseline ou esconder erros.
