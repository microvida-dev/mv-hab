...
Esta sprint pertence à fase de atribuição final da plataforma municipal de Arrendamento Acessível.

A Sprint 12 deve ser executada depois da Sprint 11, usando listas definitivas publicadas/bloqueadas, candidaturas admitidas, ranking final e regras de tipologia para gerir a afetação das habitações disponíveis aos candidatos.
...
Sprint 12 — Atribuição de Habitações
Sprint 13 — Contrato de Arrendamento
```
...
Execute atribuição por classificação
Execute atribuição por sorteio quando aplicável
Registe sorteios de forma auditável
Gere propostas de atribuição
Notifique candidatos da atribuição
Permita aceitação ou recusa pelo candidato dentro de prazo
Registe desistências
Avance automaticamente para suplentes quando houver recusa/desistência
Mantenha lista de reserva/suplentes
Bloqueie habitações atribuídas
Reabra habitações em caso de recusa/desistência
Gere ata/relatório de atribuição
Prepare candidaturas atribuídas para contrato na Sprint 13
```

Esta sprint deve gerir a decisão operacional de afetação de habitações, mas não deve criar contrato de arrendamento, caução, pagamentos ou gestão pós-contratual.

---

# 2. Instrução operacional para Codex

Executa apenas esta Sprint 12.
...
Se `ApplicationScore`, `RankingEntry` ou ranking final equivalente não existir, interrompe a implementação funcional e informa:
...
A atribuição deve usar apenas listas definitivas publicadas, bloqueadas ou formalmente aprovadas.
...
Não hardcodar regras de tipologia, sorteio, prazos de aceitação ou consequências de recusa sem validação jurídica.
...
Todos os sorteios devem preservar participantes, seed, ordem e resultado.
Relatórios/atas devem ser gerados e preservados.
...
→ Se recusar/desistir: chamar suplente
...
Se não houver regra aplicável, usar requires_manual_review em vez de atribuição automática.
```

---

## 10.3 HousingPreference
...
## 10.14 AllocationNotification

Criar entidade, se `OfficialNotification` não for suficiente:
...
## 13.1 ContestHousingUnitService
...
Devolver adequado, inadequado ou requires_manual_review
```
...
## 13.4 AllocationRuleSetResolver
...
## 13.7 PreferenceAllocationService
...
## 13.11 AllocationResponseService
...
## 13.15 AllocationNotificationService
...
## 13.16 ContractReadinessService
...
### TypologyAdequacyRuleController
...
# 21. Regras de atribuição por ranking
...
Atribuir primeira habitação adequada disponível
Criar reserve list para candidatos não atribuídos
...
# 22. Regras de sorteio auditável
...
# 23. Regras de aceitação, recusa e suplentes
...
# 24. Integração com listas definitivas
...
# 25. Integração com contrato — Sprint 13
...
23. Recomendação objetiva para avançar ou não para Sprint 13
```
...
A Sprint 12 só está concluída quando a plataforma permitir atribuir habitações a candidatos com base em lista definitiva, ranking, tipologia e/ou sorteio auditável, permitindo aceitação, recusa, substituição por suplente e geração de relatório de atribuição.

O resultado deve permitir que a Sprint 13 crie contratos apenas para atribuições aceites e marcadas como prontas para contrato.

Fim da Sprint 12.

---

# Execução técnica em 12/06/2026

## Implementado

- Módulo de atribuição de habitações baseado em listas definitivas aprovadas/publicadas/bloqueadas.
- Associação de habitações existentes a programa/concurso.
- Regras configuráveis de adequação de tipologia por programa/concurso.
- Preferências de habitação do candidato por candidatura própria pronta para atribuição.
- Regras de atribuição com método por ranking, preferências ou sorteio.
- Execução de atribuição com bloqueio de duplicados ativos por candidatura e habitação.
- Ofertas de atribuição com prazo, aceitação, recusa, expiração e desistência.
- Lista suplente/reserva e chamada de suplente quando configurada.
- Sorteio auditável com seed, algoritmo, participantes, ordem, resultado e hash.
- Relatório/ata preliminar de atribuição.
- Notificações oficiais internas/in-app reutilizando `official_notifications`.
- Scopes `readyForContract()` para preparar a Sprint 13 sem criar contrato.
- Backoffice e área do candidato com navegação por role.
- Factories e testes mínimos do Sprint 12.
...
- Migration `2026_06_12_010000_create_allocation_tables.php`.
- Models, enums, policies, Form Requests e services do domínio `Allocation`.
- Controllers backoffice e candidate para atribuição, preferências, ofertas, sorteios, suplentes e relatórios.
- Views Blade em `resources/views/backoffice/allocation/*` e `resources/views/candidate/*`.
- Factories para os novos models.
- Teste `tests/Feature/Sprint12AllocationTest.php`.

## Problemas encontrados e correções

- Alguns services usavam `create()` com campos protegidos por mass assignment (`run_number`, estados, ownership, timestamps, `application_id` em preferências). Corrigido para criar entidades via model e escrever campos críticos com `forceFill()` no service.
- `allocation_offers` não possui `contest_id` direto; controllers/views usam `allocation.contest` e `allocation.program`.
- Factories foram ajustadas para enums reais (`ReserveListStatus::Active`, `LotteryResultType::Selected`).
...
- `php artisan migrate`: migration incremental do Sprint 12 aplicada com sucesso.
- `php artisan route:list`: executado com sucesso; 440 rotas carregadas.
- `php artisan test --filter=Sprint12AllocationTest`: passou com 6 testes/33 asserções.
- `php artisan test`: passou com 118 testes/672 asserções antes e depois de Pint.
- `npm run build`: build Vite concluído com sucesso.
- `./vendor/bin/pint`: executado; aplicou formatação.
- `./vendor/bin/pint --test`: passou.
- Verificação de `vendor/bin` para `phpstan`/`psalm`: não existem, por isso não foram executados.

## Fora de âmbito confirmado

- Não foi implementado contrato.
- Não foi implementado cálculo de renda.
- Não foi implementada caução.
- Não foram implementados pagamentos.
- Não foi implementada manutenção.
- Não foram implementadas notificações externas reais por email/SMS.
- Não foram criadas integrações externas, assinatura digital ou geração de PDF oficial.

## Pendências para Sprint 13

- Criar contrato apenas para `allocations.status = ready_for_contract`.
- Validar juridicamente minuta contratual, base legal, caução, prazos e cálculo de renda.
- Definir relação contratual com habitação atribuída sem recalcular atribuição.
- Bloquear criação de contrato para atribuições recusadas, expiradas, desistidas ou substituídas.

## Validações jurídicas pendentes

- Regras reais de tipologia e ocupação.
- Consequências administrativas de recusa, expiração e desistência.
- Critérios de sorteio, publicitação de seed/hash e forma de ata.
- Textos oficiais de oferta, chamada de suplente e relatório.
- Retenção/anonimização de preferências, sorteios e relatórios.
