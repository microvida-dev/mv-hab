# MASTER PROMPT — EXECUÇÃO DA SPRINT 17: RELATÓRIOS, INDICADORES E DASHBOARD EXECUTIVO
...
Esta sprint pertence à camada estratégica, operacional e analítica da plataforma municipal de Arrendamento Acessível.

A Sprint 17 deve criar dashboards, indicadores, relatórios e exportações que permitam à equipa municipal acompanhar candidaturas, elegibilidade, documentos, reclamações, habitações, atribuições, contratos, rendas, manutenção e custos sem consultar candidatura a candidatura.

---

# 1. Regra principal

Executa apenas a Sprint 17.
...
Se este ficheiro não existir, interrompe e informa que falta o ficheiro de definição da Sprint 17.
...
A Sprint 17 deve ser tolerante a dependências parciais.

Se um módulo ainda não existir, não criar dados fictícios para simular produção. Implementar a infraestrutura de reporting e marcar o indicador como indisponível por dependência em falta.

Exemplos:

```text
Se Application não existir, indicadores de candidaturas ficam indisponíveis.
Se HousingUnit não existir, indicadores de habitação ficam indisponíveis.
Se Sprint 14 não existir, indicadores financeiros/rendas em atraso ficam indisponíveis.
Se Sprint 15 não existir, indicadores de manutenção/custos ficam indisponíveis.
```
...
Relatórios podem conter dados pessoais, dados financeiros, dados familiares, dados habitacionais e informação sensível de processos administrativos.
...
Relatórios financeiros devem exigir permissão específica.
Relatórios com dados pessoais devem exigir rastreabilidade.
...
Exportações com dados pessoais devem guardar filtros, utilizador, data, formato e âmbito.
Ficheiros exportados devem ficar em storage privado quando persistidos.
Não expor ficheiros por URL público.
Não incluir NIF, e-mail, telefone, morada ou nome completo no nome do ficheiro.
...
Integrações externas não autorizadas
```
...
→ Sistema calcula indicadores disponíveis
→ Sistema marca dependências em falta como indisponíveis
→ Sistema mostra cartões, tabelas e gráficos simples
→ Sistema regista acesso quando aplicável
```

## Execução de relatório

```text
Utilizador abre módulo de relatórios
→ Seleciona relatório
→ Define filtros
→ Sistema valida permissões
→ Sistema executa relatório
→ Sistema regista ReportRun
→ Sistema apresenta resultados
→ Sistema permite exportar apenas formatos autorizados
```

## Exportação sensível

```text
Utilizador solicita exportação
→ Sistema valida filtros
→ Sistema valida permissão de exportação
→ Sistema exige confirmação para dados sensíveis
→ Sistema gera exportação
→ Sistema guarda ficheiro em storage privado
→ Sistema regista ReportExport
→ Sistema regista download
→ Sistema cria auditoria, se existir
```

---

# 11. Indicadores obrigatórios
...
Widgets devem ser configuráveis e protegidos por permissões quando necessário.
```

---

## 13.3 IndicatorDefinition
...
Indicador com dependência ausente deve devolver pending_dependency.
```

---

## 13.4 IndicatorSnapshot
...
```text
Snapshots são opcionais e servem performance/histórico.
Snapshots não devem esconder dados atualizados quando cálculo real for necessário.
```

---

## 13.5 ReportDefinition
...
Para relatórios sensíveis, criar auditoria adicional se existir.
```

---

## 13.10 ReportAccessLog
...
```text
Registar acesso ao dashboard executivo.
Registar acesso a relatórios sensíveis.
Registar execuções, exportações e downloads.
Não guardar datasets completos no log.
```

---

# 14. Enums obrigatórios
...
## 16.2 ExecutiveDashboardService
...
## 16.3 OperationalDashboardService
...
## 16.4 IndicatorCalculationService
...
## 16.10 SensitiveDataMaskingService
...
Pode gerir widgets.
Pode exportar relatórios conforme policy.
...
Não permitir mass assignment de sensibilidade.
...
Fim da master prompt da Sprint 17.
