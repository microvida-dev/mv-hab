# MASTER PROMPT — EXECUÇÃO DA SPRINT 24: BACKOFFICE OPERACIONAL E GESTÃO DO PROCEDIMENTO

Atua como arquiteto sénior Laravel, tech lead, product engineer, QA engineer, especialista em backoffice municipal, gestão documental, relatórios, indicadores, automatizações administrativas e plataformas públicas de Habitação/Arrendamento Acessível.
...
Esta sprint pertence à fase de reforço operacional do backoffice municipal, com foco em eficiência administrativa, dashboards, métricas, relatórios, standardização documental, alertas internos, minutas, listas provisórias/finais, atas e confirmações automáticas com número de processo.

A implementação deve preservar os módulos existentes de registo, simulador, candidaturas, gestão documental, workflow administrativo, acompanhamento processual, visitas, tickets, notificações, audiência prévia, reclamações, listas, classificação, auditoria, RGPD e relatórios.

---

# 1. Regra principal

Executa apenas a Sprint 24.

Não avances para Sprint 25, Sprint 26 ou qualquer sprint futura sem validação explícita.
...
Se este ficheiro não existir, interrompe e informa que falta o ficheiro de definição da Sprint 24.
...
Sistema de visitas
Sistema de tickets/apoio
Sistema de notificações
Sistema de alertas, se existir
Sistema de workflow administrativo
Sistema de acompanhamento processual/timeline
...
Não duplicar módulos já existentes de relatórios, notificações, listas, documentos, atas ou auditoria.
...
Não destruir snapshots existentes.
```

Se minutas já existirem:

```text
Reutilizar templates, variáveis e mecanismo de geração existente.
Não criar sistema paralelo de minutas se existir mecanismo configurável.
```

Se acompanhamento processual existir:

```text
Integrar confirmações, atas, relatórios e listas com timeline/process events quando aplicável.
```
...
Publicação automática de listas não deve ocorrer sem validação humana, salvo regra já existente e explícita.
```
...
```text
Técnico autorizado acede ao backoffice
→ Sistema mostra fila de trabalho
→ Sistema mostra candidaturas sem técnico ou com ação pendente
→ Sistema mostra documentos por validar
→ Sistema mostra visitas do dia/semana
→ Sistema mostra tickets pendentes
→ Sistema mostra alertas atribuídos
→ Sistema permite navegar para os processos respetivos
```

## 11.3 Relatório por candidatura
...
## 11.4 Standardização documental
...
## 11.9 Confirmação automática com número de processo
...
## 13.1 BackofficeDashboardSnapshot
...
## 13.7 GeneratedProcedureDocument
...
## 15.1 ExecutiveDashboardService
...
## 15.2 OperationalDashboardService
...
## 15.3 DashboardMetricAggregator
...
## 15.5 DeadlineStatisticsService

Responsável por:

```text
Calcular prazos ativos
Calcular prazos a expirar
Calcular prazos expirados
Agrupar por concurso/candidatura/tipo
Gerar payload para dashboard e alertas
```

## 15.6 ApplicationReportService
...
## 15.7 ApplicationReportPayloadBuilder

Responsável por:

```text
Construir payload normalizado do relatório
Carregar relações com eager loading
Mascarar campos conforme policy
Preparar dados para HTML/PDF/CSV/XLSX
```

## 15.8 ApplicationReportExportService

Responsável por:

```text
Exportar relatório em HTML
Exportar PDF se infraestrutura existir
Exportar CSV/XLSX se aplicável
Guardar ficheiro privado
Não expor path
```

## 15.9 DocumentDossierService
...
## 15.10 DocumentDossierBuilder

Responsável por:

```text
Carregar checklist documental
Carregar documentos submetidos
Agrupar por categoria
Ordenar por regra
Gerar itens do dossier
Gerar contadores
```

## 15.11 DocumentStandardizationService
...
Atribuir alerta a utilizador/role
Listar alertas
Marcar como visto
Resolver
Dispensar
...
## 15.14 ProcedureTemplateService
...
## 15.15 TemplateVariableResolver
...
## 15.16 TemplateRenderingService
...
## 15.17 ProvisionalListAutomationService
...
## 15.18 DefinitiveListAutomationService
...
## 15.19 ListAutomationValidator

Responsável por:

```text
Validar pré-condições de listas
Validar candidaturas incompletas
Validar reclamações pendentes
Validar classificação/ranking
Gerar erros e warnings explicáveis
```

## 15.20 ProcedureMinuteService
...
## 15.21 ProcedureMinutePayloadBuilder

Responsável por:

```text
Construir payload de ata
Carregar dados de concurso/procedimento
Carregar listas/ranking quando aplicável
Carregar ações relevantes
Gerar resumo do procedimento
```

## 15.22 ProcessNumberGenerator
...
## 15.23 AutomaticProcessConfirmationService
...
Não colocar queries agregadas pesadas diretamente em controllers.
...
## 17.1 FilterExecutiveDashboardRequest
...
## 17.2 GenerateApplicationReportRequest
...
## 17.3 GenerateDocumentDossierRequest
...
## 17.4 StoreProcedureTemplateRequest
...
## 17.5 RunProvisionalListAutomationRequest
...
## 17.6 RunDefinitiveListAutomationRequest
...
## 17.7 GenerateProcedureMinuteRequest
...
## 17.8 GenerateProcessConfirmationRequest
...
Downloads de relatórios/dossiers/documentos gerados exigem autorização.
```
...
## 21.3 Relatório por candidatura
...
# 22. Regras de dashboards e métricas
...
Evitar N+1.
Evitar carregar listas completas sem paginação.
```
...
# 23. Regras de relatórios por candidatura
...
Relatório deve ser reprodutível com payload/snapshot.
```
...
# 24. Regras de standardização documental
...
Alertas internos nunca devem aparecer ao candidato.
```

---

# 26. Regras de minutas
...
# 27. Regras de automatização de listas
...
# 29. Regras de confirmações automáticas com número de processo
...
## 33.2 Relatórios por candidatura
...
## 33.3 Dossier documental e standardização
...
## 33.5 Minutas e documentos gerados
...
# 34. PHPStan específico da Sprint 24
...
Fim da master prompt da Sprint 24.
