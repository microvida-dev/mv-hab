# MASTER PROMPT — EXECUÇÃO DA SPRINT 15: MANUTENÇÃO, VISTORIAS E GESTÃO DO IMÓVEL
...
A Sprint 15 deve evoluir o módulo atual de manutenção para um módulo completo de gestão técnica da habitação, incluindo pedidos de manutenção pelo arrendatário, classificação de urgência, atribuição a técnico ou fornecedor, intervenções, vistorias, autos, fotografias/anexos, custos, indicadores e histórico técnico completo do imóvel.

---

# 1. Regra principal

Executa apenas a Sprint 15.
...
Se este ficheiro não existir, interrompe e informa que falta o ficheiro de definição da Sprint 15.
...
Se não existir `HousingUnit` ou entidade equivalente, interrompe a implementação funcional e informa:
...
Se existir `LeaseContract`, `Contract` ou equivalente, os pedidos do arrendatário devem ser associados ao contrato ativo e à habitação contratualizada.

Se não existir contrato, ainda é possível implementar a gestão técnica por imóvel, mas o acesso do arrendatário deve ficar limitado e documentado.

Documentar nesse caso:
...
Se a Sprint 6 existir, anexos e fotografias devem reutilizar o sistema documental existente sempre que possível.

Se não existir, criar storage privado próprio para anexos técnicos, sem usar `public/storage` para documentos sensíveis.

---

# 6. Validação jurídica, operacional e RGPD

Esta sprint lida com dados pessoais, fotografias, habitações e situações potencialmente sensíveis.
...
Textos de autos, notificações e relatórios devem ser parametrizáveis ou tratados como minutas sujeitas a validação municipal.
...
## 12.4 MaintenanceRequestStatusHistory
...
## 12.9 InspectionChecklistTemplate
...
## 12.10 InspectionChecklistTemplateItem
...
## 12.13 PropertyInspectionAttachment
...
## 12.14 PropertyInspectionReport
...
hasMany MaintenanceCost
morphMany PropertyHistoryEvent as source
```
...
## PropertyInspectionReportService
...
## PropertyTechnicalHistoryService
...
# 22. UX obrigatória para arrendatário
...
# 26. Histórico técnico do imóvel
...
Fim da master prompt da Sprint 15.
