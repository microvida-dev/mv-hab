# MASTER PROMPT — EXECUÇÃO DA SPRINT 26: ÁREA DO INQUILINO E GESTÃO PÓS-ATRIBUIÇÃO

Atua como arquiteto sénior Laravel, tech lead, product engineer, QA engineer, especialista em exploração habitacional municipal, gestão pós-contratual, arrendamento acessível, contratos, faturação, pagamentos, manutenção, vistorias, comunicações, RGPD, auditoria e plataformas públicas de Habitação/Arrendamento Acessível.
...
Esta sprint pertence à fase de exploração habitacional e gestão pós-atribuição, com foco na consolidação da área do inquilino, consulta de contratos, faturas, pagamentos, cobranças internas, manutenção, vistorias, comunicações, histórico de intervenções, relatórios de manutenção e dashboard operacional do senhorio/município.

A implementação deve preservar os módulos existentes de registo, simulador, candidaturas, gestão documental, workflow administrativo, classificação, ranking, listas, sorteios, atribuição, contratos, pagamentos, notificações, acompanhamento processual, relatórios, atas, auditoria, RGPD, manutenção, vistorias e área do inquilino.

---

# 1. Regra principal

Executa apenas a Sprint 26.

Não avances para Sprint 27, Sprint 28 ou qualquer sprint futura sem validação explícita.
...
Se este ficheiro não existir, interrompe e informa que falta o ficheiro de definição da Sprint 26.
...
Não criar sistema paralelo incompatível com a área do inquilino existente.
...
Se o módulo de auditoria existir:

```text
Auditar consulta e alterações críticas.
Registar downloads sensíveis.
Separar ações de leitura, criação, alteração e exportação.
```
...
Inquilino só vê recibos/comprovativos próprios.
...
Notas internas nunca devem aparecer ao inquilino.
```
...
A plataforma deve permitir ao inquilino:

```text
Consultar contratos ativos e históricos
Consultar documentos contratuais autorizados
Consultar faturas/rendas emitidas
Consultar estado dos pagamentos
Descarregar recibos/comprovativos autorizados
Submeter pedidos de manutenção
Acompanhar estado dos pedidos
Consultar agendamentos de vistorias
Receber comunicações do município
Responder a comunicações quando aplicável
Consultar histórico de intervenções
Consultar notificações relacionadas com contrato, pagamentos, manutenção e vistorias
```

A plataforma deve permitir ao município/senhorio:

```text
Consultar dashboard operacional de exploração habitacional
Acompanhar contratos ativos
Acompanhar rendas/faturas emitidas, vencidas e pagas
Controlar cobranças internas automáticas
Registar pagamentos manuais ou importados
Acompanhar incumprimentos
Receber e tratar pedidos de manutenção
Planear vistorias
Atribuir técnicos/intervenções
Registar intervenções realizadas
Gerar relatórios de manutenção
Comunicar com inquilinos
...
## 11.1 Acesso à área do inquilino
...
## 11.3 Consulta de faturas e pagamentos
...
## 11.4 Cobranças automáticas internas
...
## 11.9 Comunicação município-inquilino
...
## 11.10 Dashboard operacional do senhorio
...
## 13.8 TenantMaintenanceAttachment
...
## 13.13 TenantCommunicationMessage
...
## 13.14 LandlordDashboardSnapshot
...
Evitar queries N+1 em todos os dashboards.
...
## 15.1 TenantPortalAccessService
...
## 15.4 TenantDocumentAccessService

Responsável por:

```text
Validar download de documentos contratuais
Validar download de faturas
Validar download de recibos
Validar download de relatórios
Registar acesso sensível se existir auditoria
Nunca devolver path direto
```

## 15.5 TenantInvoiceService
...
## 15.7 TenantReceiptService

Responsável por:

```text
Gerar recibo/comprovativo interno quando aplicável
Guardar ficheiro privado
Permitir download autorizado
Auditar geração e download
```

## 15.8 TenantChargeRunService
...
## 15.9 AutomaticTenantChargeService
...
## 15.10 TenantDebtStatusService

Responsável por:

```text
Calcular dívida atual
Calcular valores vencidos
Calcular pagamentos parciais
Gerar indicadores para dashboard
Gerar alertas internos quando aplicável
```

## 15.11 TenantMaintenanceRequestService
...
## 15.12 TenantMaintenanceAttachmentService

Responsável por:

```text
Validar uploads
Guardar ficheiros em storage privado
Calcular checksum
...
## 15.13 MaintenanceInterventionService
...
## 15.14 MaintenanceReportService
...
## 15.15 MaintenanceNotificationService

Responsável por:

```text
Notificar pedido recebido
Notificar pedido atualizado
Notificar intervenção agendada
Notificar intervenção concluída
Não enviar SMS/e-mail real sem configuração segura
```

## 15.16 TenantInspectionService
...
## 15.17 TenantInspectionSchedulingService
...
## 15.18 TenantInspectionReportService
...
## 15.19 TenantCommunicationService
...
## 15.20 TenantCommunicationMessageService
...
## 15.21 TenantCommunicationNotificationService

Responsável por:

```text
Notificar nova comunicação do município
Notificar resposta do inquilino
Registar falhas de envio
Integrar com notifications existentes quando possível
```

## 15.22 LandlordDashboardService
...
## 15.23 LandlordMetricAggregator
...
## 15.24 LandlordAlertService

Responsável por:

```text
Detetar faturas vencidas
Detetar contratos a terminar
Detetar pedidos urgentes
Detetar vistorias pendentes
Criar alertas internos se módulo existir
```
...
Pagamentos, cobranças, manutenção, vistorias, documentos e comunicações devem usar policies.
...
## 17.1 StoreTenantMaintenanceRequestRequest
...
## 17.2 UpdateMaintenanceRequestStatusRequest
...
## 17.3 StoreMaintenanceInterventionRequest
...
## 17.4 StoreTenantInspectionRequest
...
## 17.5 RegisterTenantPaymentRequest
...
## 17.6 RunTenantChargeRunRequest
...
## 17.7 StoreTenantCommunicationRequest
...
## 17.8 StoreTenantCommunicationMessageRequest
...
## 17.9 FilterLandlordDashboardRequest
...
# 19. Rotas da área do inquilino obrigatórias
...
# 20. Rotas de backoffice obrigatórias
...
## 22.4 Pedido de manutenção — inquilino
...
## 22.5 Pedido de manutenção — backoffice
...
## 22.8 Dashboard operacional do senhorio
...
Consulta de contrato sensível deve ser auditada se existir módulo de auditoria.
```

---

# 24. Regras de faturas e pagamentos
...
# 28. Dashboard operacional do senhorio
...
## 33.3 Faturas, pagamentos e cobranças
...
## 33.7 Dashboard operacional do senhorio
...
# 34. PHPStan específico da Sprint 26
...
Fim da master prompt da Sprint 26.
