...
A Sprint 11 deve ser executada depois da Sprint 10, utilizando o ranking interno, as pontuações e as decisões administrativas já preparadas nas sprints anteriores para gerar listas provisórias, gerir reclamações, promover audiência de interessados quando aplicável, decidir reclamações e produzir listas definitivas com histórico integral.
...
Separe candidaturas admitidas e excluídas
...
Registe decisões administrativas
Gere notificações oficiais internas
Prepare a Sprint 12 para atribuição
```

Esta sprint deve criar um módulo formal, auditável e juridicamente cauteloso para a transição entre ranking interno e decisão final.

---

# 1A. Estado de execução — 12/06/2026

## Implementado

```text
Listas provisórias geradas a partir de RankingSnapshot interno/bloqueado
Entradas de lista com identificador público pseudonimizado
Aprovação obrigatória antes de publicação
Publicações versionadas por canal em list_publications
Portal público de resultados anonimizados
Consulta autenticada do candidato aos próprios resultados
Reclamações próprias durante prazo ativo
Anexos de reclamação por referência a DocumentSubmission existente
Análise, receção, revisão e decisão fundamentada de reclamações no backoffice
Pedidos de informação complementar e resposta do candidato
Audiência de interessados, pronúncia do candidato e revisão backoffice
Geração de lista definitiva bloqueada por reclamações/audiências pendentes
Registo de efeitos em list_change_logs
Notificações oficiais internas/in-app, sem email/SMS real
Preparação técnica para Sprint 12 através de listas definitivas publicadas/bloqueadas
```

## Fora de âmbito respeitado

```text
Não foi implementada atribuição de habitações
Não foi implementado sorteio
Não foram implementados contratos, pagamentos ou caução
Não foram enviadas notificações reais por email/SMS
Não foi recalculado ranking automaticamente por reclamação
Não foram criadas integrações externas
Não foram introduzidos dados pessoais reais, passwords, tokens ou APP_KEY
```

## Ficheiros criados

```text
database/migrations/2026_06_12_000000_create_list_publication_tables.php

app/Enums/*List*, *Complaint*, *Hearing*, *OfficialNotification*, *AdditionalInformation*
...
app/Models/ComplaintReview.php
app/Models/ComplaintDecision.php
...
app/Policies/*List*, *Complaint*, *Hearing*, *OfficialNotification*, *AdditionalInformation*
app/Http/Requests/*List*, *Complaint*, *Hearing*, *OfficialNotification*, *AdditionalInformation*
app/Http/Controllers/Backoffice/*List*, *Complaint*, *Hearing*, *OfficialNotification*, *AdditionalInformation*
app/Http/Controllers/Candidate/*Complaint*, *Hearing*, *OfficialNotification*, PublishedListController.php
app/Http/Controllers/PublicArea/PublishedResultListController.php

resources/views/backoffice/lists/**
resources/views/backoffice/complaints/**
resources/views/backoffice/complaint-decisions/**
resources/views/backoffice/additional-information-requests/**
resources/views/backoffice/hearings/**
resources/views/backoffice/official-notifications/**
resources/views/candidate/results/**
resources/views/candidate/complaints/**
resources/views/candidate/additional-information/**
resources/views/candidate/hearings/**
resources/views/candidate/official-notifications/**
resources/views/public/results/**

database/factories/*List*, *Complaint*, *Hearing*, *OfficialNotification*, *AdditionalInformation*
tests/Feature/Sprint11ListsComplaintsHearingTest.php
```
...
app/Models/User.php
app/Services/Candidate/RegistrationProgressService.php (normalização Pint)
...
GET/POST/PATCH /area-candidato/reclamacoes...
GET/POST /area-candidato/pedidos-informacao-complementar/{additionalInformationRequest}/responder
GET/POST /area-candidato/audiencias...
GET/POST /area-candidato/notificacoes-oficiais...

GET/POST /backoffice/lists/provisional...
GET/POST /backoffice/lists/definitive...
GET/POST /backoffice/complaints...
GET/POST /backoffice/complaint-decisions...
GET/POST /backoffice/additional-information-requests...
GET/POST /backoffice/hearings...
GET/POST /backoffice/hearing-submissions...
GET/POST /backoffice/official-notifications...
```
...
php artisan migrate
php artisan test tests/Feature/Sprint11ListsComplaintsHearingTest.php
php artisan test
npm run build
./vendor/bin/pint
```
...
php artisan route:list: 374 rotas
Sprint11ListsComplaintsHearingTest: 6 testes / 42 asserções
php artisan test: 112 testes / 639 asserções
npm run build: Vite build concluído
./vendor/bin/pint: executado e ficheiros normalizados
phpstan/psalm: não existem em vendor/bin; não executados
```

## Pendências

```text
Validação jurídica final do formato público das listas
Validação dos prazos reais de reclamação e audiência
Validação dos textos oficiais de publicação, reclamação, audiência e decisão
Prova externa de envio e templates oficiais na Sprint 16
Retenção, anonimização, exportação e DPIA na Sprint 18
Definição operacional do universo que avança para atribuição na Sprint 12
```
...
Não criar candidatura nesta sprint.
...
Se `RankingSnapshot` ou `ApplicationScore` não existir, interrompe a implementação funcional e informa:

```text
A Sprint 11 depende da Sprint 10 — Matriz de Classificação e Ranking.
```

Não criar matriz de classificação ou ranking simplificado dentro desta sprint.

A Sprint 11 deve usar apenas rankings internos concluídos e, preferencialmente, bloqueados.

---

# 4. Validação jurídica obrigatória
...
Não implementar publicações irreversíveis sem etapa de confirmação.

Não enviar notificações reais por email/SMS sem módulo seguro e sem validação.

Não expor dados pessoais sem regras de anonimização.

Não publicar listas definitivas sem histórico da lista provisória, reclamações e decisões.
...
Audiência de interessados deve ficar registada quando aplicável.
Pronúncias devem ficar registadas.
Listas definitivas devem preservar histórico das alterações.
Publicações devem ser versionadas.
Notificações devem ficar registadas.
Dados pessoais devem ser minimizados.
Anonimização deve ser configurável.
```

Qualquer texto oficial deve ser tratado como minuta configurável e validável juridicamente.
...
Entradas de lista
Versões de lista
Publicação controlada de listas
...
Preparação da atribuição na Sprint 12
Policies
Form Requests
Services
Rotas
...
Podem ser criados pontos de integração para estas funcionalidades futuras, mas não implementar a Sprint 12 ou fases seguintes.
...
A Sprint 11 não decide a habitação atribuída.

A Sprint 11 não celebra contratos.

A Sprint 11 apenas consolida o resultado administrativo final do concurso.
...
Se o projeto ainda não tiver envio real, usar apenas estados internos e não marcar `sent` sem envio efetivo.
...
Não alterar diretamente lista publicada; criar nova versão/superseded quando necessário.
Usar soft deletes.
```

---

## 9.2 ProvisionalListEntry
...
```text
Entradas de admitidos devem poder conter rank_position.
Entradas de excluídos devem conter motivo/fundamento.
public_identifier não deve expor NIF, email ou dados sensíveis.
...
Deve permitir identificar alterações face à lista provisória.
...
## 9.10 AdditionalInformationRequest
...
## 9.11 AdditionalInformationResponse
...
Usar um identificador público não reversível pelo público geral:
...
hasMany Complaint
hasMany Hearing
hasMany ListPublication as publications
...
Publicar em área de candidato se aplicável
...
## 13.9 AdditionalInformationService
...
## 13.11 OfficialNotificationService
...
## 13.12 FinalDecisionReadinessService

Responsável por:

```text
Verificar se a lista definitiva está publicada
...
publish
openComplaintPeriod
closeComplaintPeriod
cancel
archive
```

### DefinitiveListController
...
### OfficialNotificationController
...
A página de reclamação deve mostrar copy obrigatório:
...
# 22. Integração com workflow administrativo
...
Usar políticas documentais existentes.
```
...
# 24. Integração com notificações
...
# 31. Atualização documental obrigatória
...
# 33. Resposta final esperada do Codex
...
21. Confirmação de que não foram implementadas funcionalidades fora de âmbito
22. Recomendação objetiva para avançar ou não para Sprint 12
```
...
A Sprint 11 só está concluída quando a plataforma permitir ao Município publicar uma lista provisória controlada, permitir ao candidato reclamar, permitir ao técnico decidir reclamações, promover audiência quando aplicável e gerar uma lista definitiva com histórico completo das alterações.

O resultado deve permitir que a Sprint 12 utilize exclusivamente listas definitivas publicadas/bloqueadas para iniciar o processo de atribuição de habitações.
