...
class DrawAttendanceService
{
    ...
    public function register(LotteryDraw $draw, array $data, User $actor): DrawAttendance
    {
        return DB::transaction(function () use ($draw, $data, $actor): DrawAttendance {
            ...
            return $this->persist($lockedDraw, $data, $actor);
        });
    }

    /**
     * @param  list<array<string, mixed>>  $attendances
     * @return Collection<int, DrawAttendance>
     */
    public function registerMany(LotteryDraw $draw, array $attendances, User $actor): Collection
    {
        return DB::transaction(function () use ($draw, $attendances, $actor): Collection {
            $lockedDraw = LotteryDraw::query()->lockForUpdate()->findOrFail($draw->id);
            $records = new Collection;

            foreach ($attendances as $attendance) {
                $records->push($this->persist($lockedDraw, $attendance, $actor));
            }
     ...
     * @param  array<string, mixed>  $data
     */
    private function persist(LotteryDraw $draw, array $data, User $actor): DrawAttendance
    {
