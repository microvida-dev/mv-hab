...
class WorkTaskController extends Controller
{
    ...
    {
        Gate::authorize('viewAnyBackoffice', WorkTask::class);
    ...
    {
        Gate::authorize('viewBackoffice', $workTask);

        $workTask->load(['municipalTeam', 'assignedUser', 'histories.actor']);

        return view('backoffice.work-tasks.show', [
            'task' => $workTask,
            'teams' => $this->municipalScope
                ->municipalTeams(MunicipalTeam::query(), $this->currentUser())
                ->where('status', 'active')
                ->orderBy('name')
                ->get(),
    ...
    public function reassign(ReassignWorkTaskRequest $request, WorkTask $workTask): RedirectResponse
    {
        ...
        $team = isset($validated['municipal_team_id'])
            ? $this->municipalScope
                ...
                ->findOrFail((int) $validated['municipal_team_id'])
            : null;
        $assignee = isset($validated['assigned_user_id'])
            ? $this->municipalScope
                ...
                ->findOrFail((int) $validated['assigned_user_id'])
            : null;
    ...
    private function filteredTasks(Request $request): Builder
    {
        ...
        return $this->municipalScope
            ->workTasks(
                $this->dashboardService->visibleQuery($user),
                $user,
            )
