...
class MaintenanceAttachmentController extends Controller
{
    ...
    {
        Gate::authorize('create', MaintenanceAttachment::class);
        $this->attachments->storeForRequest($maintenanceRequest, $request->file('attachment'), $this->authenticatedUser($request), (bool) $request->boolean('visible_to_tenant'), $request->input('description'));
