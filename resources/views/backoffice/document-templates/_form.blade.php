...
<div class="grid gap-5 md:grid-cols-2">
    ...
    <div class="md:col-span-2"><x-input-label for="description" value="Descrição interna" /><textarea id="description" name="description" rows="3" class="mv-input mt-1 w-full">{{ old('description', $documentTemplate->description ?? '') }}</textarea></div>
    <div class="md:col-span-2"><x-input-label for="header" value="Cabeçalho" /><textarea id="header" name="header" rows="3" class="mv-input mt-1 w-full">{{ old('header', $documentTemplate->header ?? '') }}</textarea></div>
    <div class="md:col-span-2"><x-input-label for="body" value="Corpo em texto" /><textarea id="body" name="body" rows="12" class="mv-input mt-1 w-full" required>{{ old('body', $documentTemplate->body ?? '') }}</textarea><p class="mt-2 text-xs text-ink-500">Use variáveis autorizadas no formato <code>@{{ codigo }}</code>.</p></div>
    <div class="md:col-span-2"><x-input-label for="html_body" value="Corpo HTML opcional" /><textarea id="html_body" name="html_body" rows="10" class="mv-input mt-1 w-full">{{ old('html_body', $documentTemplate->html_body ?? '') }}</textarea></div>
    <div class="md:col-span-2"><x-input-label for="footer" value="Rodapé" /><textarea id="footer" name="footer" rows="3" class="mv-input mt-1 w-full">{{ old('footer', $documentTemplate->footer ?? '') }}</textarea></div>
</div>
<div class="mt-6 flex flex-wrap gap-5">@foreach(['is_official' => 'Modelo oficial', 'is_default' => 'Modelo predefinido', 'requires_approval' => 'Exige aprovação'] as $field => $label)<label class="flex items-center gap-2 text-sm"><input type="hidden" name="{{ $field }}" value="0"><input type="checkbox" name="{{ $field }}" value="1" class="mv-checkbox" @checked(old($field, $documentTemplate->{$field} ?? ($field === 'requires_approval')))>{{ $label }}</label>@endforeach</div>
