<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-wrap items-start justify-between gap-4">
            <div>
                <p class="text-sm font-semibold text-mvhab-primary">Minuta de procedimento</p>
    ...
    <div class="py-8">
        <div class="mx-auto max-w-5xl space-y-6 px-4 sm:px-6 lg:px-8">
            @if (session('success'))<div class="rounded-2xl border border-mvhab-support/40 bg-mvhab-surface px-4 py-3 text-sm font-semibold text-mvhab-primary">{{ session('success') }}</div>@endif
            <section class="mv-surface p-6">
                <h2 class="text-lg font-semibold text-ink-900">Conteúdo base</h2>
                <pre class="mt-4 max-h-96 overflow-auto whitespace-pre-wrap rounded-2xl bg-ink-50 p-4 text-sm text-ink-700">{{ $procedureTemplate->content }}</pre>
            </section>
            <section class="grid gap-6 lg:grid-cols-2">
                <form method="POST" action="{{ route('backoffice.procedure-templates.preview', $procedureTemplate) }}" class="mv-surface space-y-4 p-6">
                    ...
                    <input name="application_id" placeholder="ID da candidatura" class="mv-input w-full">
                    <input name="contest_id" placeholder="ID do concurso" class="mv-input w-full">
