...
<div class="grid gap-5">
    ...
    <label class="text-sm font-semibold text-ink-700">Nome
        <input name="name" value="{{ old('name', $template?->name) }}" class="mv-input mt-1 w-full">
    </label>
    <label class="text-sm font-semibold text-ink-700">Descrição
        <textarea name="description" rows="3" class="mv-input mt-1 w-full">{{ old('description', $template?->description) }}</textarea>
    </label>
    <label class="text-sm font-semibold text-ink-700">Conteúdo
        <textarea name="content" rows="14" class="mt-1 w-full rounded-2xl border-ink-200 font-mono text-sm">{{ old('content', $template?->content ?? $defaultTemplateContent) }}</textarea>
    </label>
