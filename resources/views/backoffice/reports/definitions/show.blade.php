<x-app-layout>
    ...
    <div class="space-y-7 max-w-5xl" x-data="{ confirmed: false }">
        ...
        @if ($report->code === \App\Services\Reporting\Temporal\TemporalApplicationResultExportService::REPORT_CODE)
            <x-mv.section title="Pacote municipal temporal" description="Este relatório é gerado em fila, com uma única fonte canónica e sem degradação de formatos.">
                @can('createTemporal', \App\Models\ReportExport::class)
                    <a class="mv-button-primary inline-flex" href="{{ route('backoffice.reports.temporal-exports.create') }}">Preparar exportação temporal</a>
                @endcan
            </x-mv.section>
        @else
