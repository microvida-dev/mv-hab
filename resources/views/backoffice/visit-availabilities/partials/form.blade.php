<form method="POST" action="{{ $action }}">
    ...
    <x-mv.section title="Dados da visita aberta" description="Defina a janela de disponibilidade, o contexto e a equipa responsável.">
        <div>
        ...
        <div class="mt-5">
            ...
            <textarea id="description" name="description" rows="4" class="mv-input mt-1 w-full text-sm" placeholder="Descrição">{{ old('description', $availability?->description) }}</textarea>
        </div>
        ...
        <div class="mt-5">
            <x-mv.checkbox-card
                name="is_active"
                label="Ativa"
                :checked="old('is_active', $availability?->is_active ?? true)"
            />
        </div>
