<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-wrap items-center justify-between gap-4">
            <div><p class="text-sm font-semibold text-mvhab-primary">{{ $ruleSet->name }}</p><h1 class="mt-1 text-2xl font-semibold text-ink-900">Critérios</h1></div>
            @can('create', [\App\Models\ScoringCriterion::class, $ruleSet])
                <a href="{{ route('backoffice.scoring.criteria.create', $ruleSet) }}" class="mv-button-primary">Novo critério</a>
            @endcan
        </div>
    </x-slot>
            ...
            <table class="min-w-full divide-y divide-ink-100 text-sm">
                ...
                @forelse ($criteria as $criterion)
                    <tr>
                        ...
                        <td class="px-4 py-3 text-right"><a href="{{ route('backoffice.scoring.rules.index', $criterion) }}" class="font-semibold text-mvhab-primary">Regras</a> <span class="text-ink-300">/</span> <a href="{{ route('backoffice.scoring.criteria.edit', $criterion) }}" class="font-semibold text-mvhab-primary">Editar</a></td>
                    </tr>
                @empty
