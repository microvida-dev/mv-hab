<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-wrap items-center justify-between gap-4">
            <div><p class="text-sm font-semibold text-civic-700">{{ $criterion->name }}</p><h1 class="mt-1 text-2xl font-semibold text-ink-900">Regras de pontuação</h1></div>
            @can('create', [\App\Models\ScoringRule::class, $criterion])
                <a href="{{ route('backoffice.scoring.rules.create', $criterion) }}" class="rounded-md bg-civic-700 px-4 py-2 text-sm font-semibold text-white hover:bg-civic-800">Nova regra</a>
            @endcan
        </div>
    </x-slot>
