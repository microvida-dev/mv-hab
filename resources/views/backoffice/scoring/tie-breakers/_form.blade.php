...
<form method="POST" action="{{ $action }}" class="space-y-6">
    ...
    <div><x-input-label for="description" value="Descrição" /><textarea id="description" name="description" rows="3" class="mt-1 block w-full rounded-md border-ink-200">{{ old('description', $rule->description) }}</textarea></div>
    <div class="grid gap-5 md:grid-cols-4">
        ...
        <div>
            ...
            <select id="direction" name="direction" class="mt-1 block w-full rounded-md border-ink-200">
    ...
    <div class="flex flex-wrap gap-3"><button class="rounded-md bg-civic-700 px-4 py-2 text-sm font-semibold text-white hover:bg-civic-800">Guardar</button><a href="{{ route('backoffice.scoring.tie-breakers.index', $ruleSet) }}" class="rounded-md border border-ink-200 px-4 py-2 text-sm font-semibold text-ink-700 hover:bg-ink-50">Cancelar</a></div>
</form>
