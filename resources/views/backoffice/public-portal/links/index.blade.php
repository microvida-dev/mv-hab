            ...
            <table class="min-w-full divide-y divide-ink-100 text-sm">
                ...
                <tbody class="divide-y divide-ink-100">
                    @forelse ($links as $link)
                        <tr><td class="px-4 py-3 font-semibold text-ink-900">{{ $link->label }}</td><td class="px-4 py-3">{{ $link->category }}</td><td class="px-4 py-3">{{ $link->is_active ? 'Ativa' : 'Inativa' }}</td><td class="px-4 py-3 text-right"><a href="{{ route('backoffice.public-portal.links.edit', $link) }}" class="font-semibold text-mvhab-primary">Editar</a></td></tr>
                    @empty
