<x-app-layout>
    <x-slot name="header"><div><p class="text-sm font-semibold text-civic-700">Comunicações</p><h1 class="mt-1 text-2xl font-semibold text-ink-900">Histórico e envios</h1></div></x-slot>
    <div class="py-8"><div class="mx-auto max-w-7xl space-y-8 px-4 sm:px-6 lg:px-8">
        @can('create', \App\Models\CommunicationLog::class)
            <section class="rounded-md border border-ink-100 bg-white p-6"><h2 class="text-lg font-semibold">Comunicação manual</h2><form method="POST" action="{{ route('backoffice.communications.logs.store') }}" class="mt-5 grid gap-4 md:grid-cols-2">@csrf
                <select name="recipient_user_id" class="rounded-md border-ink-200" required><option value="">Destinatário</option>@foreach($users as $user)<option value="{{ $user->id }}">{{ $user->name }} · {{ $user->email }}</option>@endforeach</select>
                <select name="channel" class="rounded-md border-ink-200">@foreach($channels as $value => $label)<option value="{{ $value }}">{{ $label }}</option>@endforeach</select>
                <x-text-input name="event_code" value="manual.communication" placeholder="Código do evento" required />
                <select name="priority" class="rounded-md border-ink-200"><option value="normal">Normal</option><option value="high">Alta</option><option value="urgent">Urgente</option></select>
                <x-text-input name="title" placeholder="Título" required />
                <x-text-input name="subject" placeholder="Assunto" />
                <textarea name="body" rows="5" class="rounded-md border-ink-200 md:col-span-2" placeholder="Conteúdo" required></textarea>
                <label class="flex items-center gap-2 text-sm"><input type="checkbox" name="requires_acknowledgement" value="1" class="rounded border-ink-300 text-civic-700">Exige tomada de conhecimento</label>
