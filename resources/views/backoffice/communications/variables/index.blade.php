<x-app-layout>
    <x-slot name="header"><div><p class="text-sm font-semibold text-mvhab-primary">Modelos</p><h1 class="mt-1 text-2xl font-semibold text-ink-900">Variáveis autorizadas</h1></div></x-slot>
    <div class="py-8"><div class="mx-auto max-w-7xl space-y-8 px-4 sm:px-6 lg:px-8">
        @can('create', \App\Models\TemplateVariable::class)
            <form method="POST" action="{{ route('backoffice.communications.variables.store') }}" class="grid gap-4 rounded-2xl mv-surface p-5 md:grid-cols-5">@csrf
                <x-text-input name="code" placeholder="codigo_variavel" required />
                <x-text-input name="name" placeholder="Nome" required />
                <select name="variable_type" class="mv-input">@foreach($types as $value => $label)<option value="{{ $value }}">{{ $label }}</option>@endforeach</select>
