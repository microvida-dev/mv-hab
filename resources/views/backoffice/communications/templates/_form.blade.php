...
<div class="grid gap-5 md:grid-cols-2">
    ...
    <div>
        ...
        <select id="template_type" name="template_type" class="mt-1 w-full rounded-md border-ink-200">
    ...
    <div>
        ...
        <select id="channel" name="channel" class="mt-1 w-full rounded-md border-ink-200">
    ...
    <div class="md:col-span-2">
        ...
        <textarea id="description" name="description" rows="3" class="mt-1 w-full rounded-md border-ink-200">{{ old('description', $notificationTemplate->description ?? '') }}</textarea>
    </div>
    <div class="md:col-span-2">
        <x-input-label for="body" value="Conteúdo em texto" />
        <textarea id="body" name="body" rows="10" class="mt-1 w-full rounded-md border-ink-200" required>{{ old('body', $notificationTemplate->body ?? '') }}</textarea>
    ...
    <div class="md:col-span-2">
        ...
        <textarea id="html_body" name="html_body" rows="8" class="mt-1 w-full rounded-md border-ink-200">{{ old('html_body', $notificationTemplate->html_body ?? '') }}</textarea>
    </div>
    <div class="md:col-span-2">
        <x-input-label for="sms_body" value="Conteúdo SMS opcional" />
        <textarea id="sms_body" name="sms_body" rows="3" class="mt-1 w-full rounded-md border-ink-200">{{ old('sms_body', $notificationTemplate->sms_body ?? '') }}</textarea>
    </div>
</div>
...
<div class="mt-6 flex flex-wrap gap-5">
    @foreach (['requires_acknowledgement' => 'Exige tomada de conhecimento', 'is_official' => 'Comunicação oficial', 'is_default' => 'Template predefinido'] as $field => $label)
        <label class="flex items-center gap-2 text-sm text-ink-700">
            ...
            <input type="checkbox" name="{{ $field }}" value="1" class="rounded border-ink-300 text-civic-700" @checked(old($field, $notificationTemplate->{$field} ?? false))>
            {{ $label }}
        </label>
    @endforeach
</div>
