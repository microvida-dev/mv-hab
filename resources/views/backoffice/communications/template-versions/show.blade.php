<x-app-layout>
    <x-slot name="header"><div><p class="text-sm font-semibold text-mvhab-primary">{{ $notificationTemplateVersion->template->name }}</p><h1 class="mt-1 text-2xl font-semibold text-ink-900">Versão {{ $notificationTemplateVersion->version_number }}</h1></div></x-slot>
    <div class="py-8"><div class="mx-auto max-w-4xl space-y-6 px-4 sm:px-6 lg:px-8">
        <section class="rounded-2xl mv-surface p-6"><p class="text-sm text-ink-500">{{ $notificationTemplateVersion->status->label() }}</p><h2 class="mt-2 text-lg font-semibold">{{ $notificationTemplateVersion->title }}</h2><p class="mt-5 whitespace-pre-line text-sm leading-7">{{ $notificationTemplateVersion->body }}</p></section>
