<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-wrap items-start justify-between gap-4">
            <div>
                <p class="text-sm font-semibold text-civic-700">Dossier documental</p>
                            ...
                            <div class="flex flex-wrap items-start justify-between gap-4">
                                ...
                                <div class="text-right text-sm">
                                    ...
                                    @if ($dossier->file_path)
                                        <a class="mt-2 inline-block font-semibold text-civic-700" href="{{ route('backoffice.document-dossiers.download', $dossier) }}">Download</a>
                                    @endif
                                </div>
                            </div>
                            <div class="mt-4 grid gap-3 sm:grid-cols-4">
                                <div class="rounded-md bg-ink-50 p-3"><p class="text-xs text-ink-500">Validados</p><p class="text-xl font-semibold">{{ $dossier->validated_documents_count }}</p></div>
                                <div class="rounded-md bg-ink-50 p-3"><p class="text-xs text-ink-500">Em falta</p><p class="text-xl font-semibold">{{ $dossier->missing_documents_count }}</p></div>
                                <div class="rounded-md bg-ink-50 p-3"><p class="text-xs text-ink-500">Rejeitados</p><p class="text-xl font-semibold">{{ $dossier->rejected_documents_count }}</p></div>
                                <div class="rounded-md bg-ink-50 p-3"><p class="text-xs text-ink-500">Expirados</p><p class="text-xl font-semibold">{{ $dossier->expired_documents_count }}</p></div>
                            </div>
                            <details class="mt-4">
                                <summary class="cursor-pointer text-sm font-semibold text-civic-700">Ver itens normalizados</summary>
