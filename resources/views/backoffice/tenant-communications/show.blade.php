<x-app-layout>
    ...
    <div class="mx-auto grid max-w-7xl gap-6 px-4 py-6 sm:px-6 lg:px-8">
        <div class="mv-card">
            <p class="text-sm text-ink-500">{{ $tenantCommunication->tenant?->name }} · {{ $tenantCommunication->status?->label() }}</p>
        ...
        <form class="mv-card grid gap-4" method="POST" action="{{ route('backoffice.tenant-operations.communications.messages.store', $tenantCommunication) }}">
            @csrf
