...
<div class="grid gap-5 md:grid-cols-2">
    <div><x-input-label for="rent_rule_set_id" value="Conjunto" /><select id="rent_rule_set_id" name="rent_rule_set_id" class="mt-1 w-full rounded-2xl border-ink-200">@foreach($ruleSets as $ruleSet)<option value="{{ $ruleSet->id }}" @selected((string) old('rent_rule_set_id', $rule->rent_rule_set_id ?? '') === (string) $ruleSet->id)>{{ $ruleSet->name }}</option>@endforeach</select></div>
    ...
    <label class="flex items-center gap-3 text-sm text-ink-700"><input type="checkbox" name="is_active" value="1" class="rounded border-ink-300 text-mvhab-primary" @checked(old('is_active', $rule->is_active ?? true))> Ativa</label>
    <div class="md:col-span-2"><x-input-label for="description" value="Descrição" /><textarea id="description" name="description" rows="3" class="mt-1 w-full rounded-2xl border-ink-200">{{ old('description', $rule->description ?? '') }}</textarea></div>
</div>
