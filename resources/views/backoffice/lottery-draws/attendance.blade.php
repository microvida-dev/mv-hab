<x-app-layout>
    <x-slot name="header"><div><p class="text-sm font-semibold text-civic-700">Sorteios</p><h1 class="mt-1 text-2xl font-semibold text-ink-900">Presenças</h1></div></x-slot>
    <div class="py-8"><div class="mx-auto max-w-7xl space-y-4 px-4 sm:px-6 lg:px-8">
        <x-flash-message />
        <div class="grid gap-3 md:grid-cols-4">@foreach($summary as $label => $value)<div class="rounded-md border border-ink-100 bg-white p-4"><p class="text-sm text-ink-500">{{ $label }}</p><p class="text-2xl font-semibold">{{ $value }}</p></div>@endforeach</div>
