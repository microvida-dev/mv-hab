<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between gap-4">
            <div>
                <p class="text-sm font-semibold text-mvhab-primary">Sorteios</p>
                <h1 class="mt-1 text-2xl font-semibold text-ink-900">Sorteios auditáveis</h1>
            </div>
            <a href="{{ route('backoffice.lottery-draws.create') }}" class="rounded-2xl bg-mvhab-primary px-4 py-2 text-sm font-semibold text-white">Novo sorteio</a>
        </div>
    </x-slot>
    ...
    <div class="py-8">
        <div class="mx-auto max-w-7xl space-y-4 px-4 sm:px-6 lg:px-8">
            <x-flash-message />
            <div class="overflow-hidden rounded-2xl border border-ink-100 bg-mvhab-card">
                    ...
                    <tbody class="divide-y divide-ink-100">
                        @forelse($lotteryDraws as $draw)
                            <tr>
                                ...
                                <td class="px-4 py-3 text-right"><a class="font-semibold text-mvhab-primary" href="{{ route('backoffice.lottery-draws.show', $draw) }}">Abrir</a></td>
                            </tr>
                        @empty
