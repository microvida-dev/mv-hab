<label class="grid gap-1 text-sm">
    ...
    <input name="name" value="{{ old('name', $team?->name) }}" class="rounded-md border-ink-200" required>
</label>
<label class="grid gap-1 text-sm">
    <span class="font-medium text-ink-700">Descrição</span>
    <textarea name="description" rows="3" class="rounded-md border-ink-200">{{ old('description', $team?->description) }}</textarea>
</label>
<div class="grid gap-4 md:grid-cols-2">
    <label class="grid gap-1 text-sm">
        <span class="font-medium text-ink-700">Estado</span>
        <select name="status" class="rounded-md border-ink-200" required>
    ...
    <label class="grid gap-1 text-sm">
        ...
        <select name="manager_user_id" class="rounded-md border-ink-200">
...
<label class="grid gap-1 text-sm">
    ...
    <textarea name="functional_scopes" rows="3" class="rounded-md border-ink-200">{{ old('functional_scopes', collect($team?->functional_scopes ?? [])->join(', ')) }}</textarea>
</label>
<label class="grid gap-1 text-sm">
