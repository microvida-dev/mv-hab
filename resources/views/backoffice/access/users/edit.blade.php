        ...
        <div class="mx-auto max-w-3xl space-y-6 px-4 sm:px-6 lg:px-8">
            ...
            <form method="POST" action="{{ route('backoffice.users.update', $user) }}" class="mv-surface grid gap-4 p-6">
                ...
                <label class="grid gap-1 text-sm">
                    ...
                    <input name="name" value="{{ old('name', $user->name) }}" class="rounded-2xl border-ink-200" required>
                </label>
                <div class="rounded-2xl bg-ink-50 p-4 text-sm text-ink-600">
                ...
                <label class="grid gap-1 text-sm">
                    ...
                    <textarea name="internal_notes" rows="3" class="rounded-2xl border-ink-200">{{ old('internal_notes', $user->internal_notes) }}</textarea>
                </label>
                <label class="grid gap-1 text-sm">
