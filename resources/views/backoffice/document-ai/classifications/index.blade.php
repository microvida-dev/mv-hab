        ...
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <form method="GET" action="{{ route('backoffice.document-ai.classifications.index') }}" class="mv-surface mb-6 grid gap-4 p-5 md:grid-cols-3 xl:grid-cols-6">
                ...
                <label class="space-y-1 text-sm">
                    ...
                    <select name="classification_status" class="w-full rounded-md border-ink-200 text-sm">
                        <option value="">Todos</option>
                ...
                <label class="space-y-1 text-sm">
                    ...
                    <select name="ocr_available" class="w-full rounded-md border-ink-200 text-sm">
                        <option value="">Todos</option>
                ...
                <label class="space-y-1 text-sm">
                    ...
                    <select name="requires_manual_review" class="w-full rounded-md border-ink-200 text-sm">
                    ...
                    <tbody class="divide-y divide-ink-100">
                        @forelse ($analyses as $analysis)
                            <tr>
                                ...
                                <td class="px-5 py-4">
                                    <span class="{{ $analysis->ocr_available ? 'text-civic-700' : 'text-amber-700' }}">
                                        {{ $analysis->ocr_available ? 'Disponível' : 'Indisponível' }}
                                    </span>
                                </td>
                                <td class="px-5 py-4 text-right">
                                    <a class="font-semibold text-civic-700" href="{{ route('backoffice.document-ai.classifications.show', $analysis) }}">Ver detalhe</a>
                                </td>
                            </tr>
                        @empty
                            <tr>
