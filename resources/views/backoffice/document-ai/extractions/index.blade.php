        ...
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <form method="GET" action="{{ route('backoffice.document-ai.extractions.index') }}" class="mv-surface mb-6 grid gap-4 p-5 md:grid-cols-3 xl:grid-cols-7">
                <label class="space-y-1 text-sm">
                    ...
                    <select name="document_type" class="w-full rounded-md border-ink-200 text-sm">
                ...
                <label class="space-y-1 text-sm">
                    ...
                    <select name="extraction_status" class="w-full rounded-md border-ink-200 text-sm">
                        <option value="">Todos</option>
                ...
                <label class="space-y-1 text-sm">
                    ...
                    <input type="text" name="field_key" value="{{ $filters['field_key'] ?? '' }}" class="w-full rounded-md border-ink-200 text-sm" placeholder="nif, name, total_income">
                </label>

                <label class="space-y-1 text-sm">
                    <span class="font-semibold text-ink-700">Confiança mín.</span>
                    <input type="number" step="0.01" min="0" max="1" name="min_confidence" value="{{ $filters['min_confidence'] ?? '' }}" class="w-full rounded-md border-ink-200 text-sm">
                </label>

                <label class="space-y-1 text-sm">
                    <span class="font-semibold text-ink-700">Confiança máx.</span>
                    <input type="number" step="0.01" min="0" max="1" name="max_confidence" value="{{ $filters['max_confidence'] ?? '' }}" class="w-full rounded-md border-ink-200 text-sm">
                </label>
                    ...
                    <tbody class="divide-y divide-ink-100">
                        @forelse ($analyses as $analysis)
                            <tr>
                                ...
                                <td class="px-5 py-4">
                                    <p class="font-semibold text-civic-800">{{ $analysis->detected_document_label ?? 'Por classificar' }}</p>
                                ...
                                <td class="px-5 py-4">
                                    <span class="{{ $analysis->extraction_requires_manual_review ? 'text-amber-700' : 'text-civic-700' }}">
                                ...
                                <td class="px-5 py-4 text-right">
                                    <a class="font-semibold text-civic-700" href="{{ route('backoffice.document-ai.extractions.show', $analysis) }}">Ver extração</a>
                                </td>
                            </tr>
                        @empty
                            <tr>
