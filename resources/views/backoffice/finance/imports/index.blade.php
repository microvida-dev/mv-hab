<x-app-layout>
    ...
    <div class="mv-card mt-4 overflow-x-auto"><table class="min-w-full text-sm"><tbody>@foreach ($batches as $batch)<tr class="border-t border-ink-100"><td class="py-2"><a class="text-civic-700" href="{{ route('backoffice.finance.imports.show', $batch) }}">{{ $batch->batch_number }}</a></td><td>{{ $batch->status->label() }}</td><td>{{ $batch->row_count }} linhas</td></tr>@endforeach</tbody></table>{{ $batches->links() }}</div>
</x-app-layout>
