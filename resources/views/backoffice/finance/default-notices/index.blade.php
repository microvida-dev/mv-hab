<x-app-layout>
    ...
    <div class="mv-card mt-4 overflow-x-auto"><table class="min-w-full text-sm"><tbody>@foreach ($notices as $notice)<tr class="border-t border-ink-100"><td class="py-2"><a class="text-civic-700" href="{{ route('backoffice.finance.default-notices.show', $notice) }}">{{ $notice->notice_number }}</a></td><td>{{ $notice->tenant?->name }}</td><td>{{ $notice->status->label() }}</td></tr>@endforeach</tbody></table>{{ $notices->links() }}</div>
</x-app-layout>
