...
<div class="rounded-lg border border-civic-100 bg-civic-50 p-4">
    ...
    @if ($action['enabled'] && $action['route'])
        <a href="{{ route($action['route'], $action['parameters'] ?? []) }}" class="mt-3 inline-flex rounded-md text-sm font-semibold text-civic-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-civic-500 focus-visible:ring-offset-2">
            Abrir ação
        </a>
    @endif
</div>
