            ...
            <div class="p-5">
                ...
                <div class="mt-4 space-y-2">
                    @foreach (($queue['items'] ?? []) as $item)
                        <a href="{{ $item['url'] }}" class="block truncate rounded-2xl border border-ink-100 px-3 py-2 text-sm font-medium text-ink-800 hover:border-mvhab-support hover:bg-mvhab-surface focus:outline-none focus:ring-2 focus:ring-mvhab-primary">
                            {{ $item['title'] }}
                        </a>
                    @endforeach
                </div>
