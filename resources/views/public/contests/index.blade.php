<x-public-layout title="Concursos" description="Concursos municipais publicados com oferta habitacional e prazos de candidatura.">
    <section class="border-b border-ink-100 bg-ink-50">
        <div class="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
            ...
            <p class="text-sm font-semibold text-civic-700">Oportunidades publicadas</p>
    ...
    <section class="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
        <form method="GET" action="{{ route('public.contests.index') }}" class="mb-8 grid gap-4 rounded-md border border-ink-100 bg-white p-5 md:grid-cols-[minmax(0,1fr)_16rem_auto]">
            <input type="search" name="q" value="{{ $filters['q'] ?? '' }}" class="rounded-md border-ink-200 text-sm" placeholder="Pesquisar por título, código ou resumo">
            <select name="status" class="rounded-md border-ink-200 text-sm">
