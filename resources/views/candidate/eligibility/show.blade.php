@php
    $resultClasses = match ($check->result) {
        \App\Enums\EligibilityResult::Eligible => 'bg-mvhab-surface text-mvhab-primary',
...
<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-wrap items-start justify-between gap-4">
            ...
            <span class="rounded-2xl px-3 py-1.5 text-sm font-semibold {{ $resultClasses }}">{{ $check->result->label() }}</span>
        </div>
    </x-slot>
                ...
                <div class="divide-y divide-ink-100">
                    @forelse ($check->results as $result)
                        <div class="flex gap-4 px-6 py-5">
                            <span class="mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-2xl {{ $result->result === \App\Enums\EligibilityCriterionResult::Passed ? 'bg-mvhab-surface text-mvhab-primary' : ($result->result === \App\Enums\EligibilityCriterionResult::Failed ? 'bg-red-50 text-red-700' : 'bg-signal-50 text-signal-700') }}">
