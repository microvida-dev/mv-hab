        ...
        <div class="mx-auto max-w-6xl space-y-6 px-4 sm:px-6 lg:px-8">
            <section class="mv-surface p-6">
                <div class="flex flex-wrap items-start justify-between gap-4">
                    ...
                    <div class="min-w-48 rounded-2xl bg-ink-50 p-4 text-sm">
                ...
                @if ($publicStatus?->next_step)
                    <p class="mt-4 rounded-2xl bg-mvhab-surface px-4 py-3 text-sm font-semibold text-mvhab-primary">Próximo passo: {{ $publicStatus->next_step }}</p>
                @endif
                <a href="{{ route('candidate.processes.timeline', $process) }}" class="mt-4 inline-flex text-sm font-semibold text-mvhab-primary">Ver timeline completa</a>
            </section>
                ...
                <div class="mt-4 grid gap-3 md:grid-cols-2">
                    @forelse ($actions as $action)
                        <div class="rounded-2xl border border-ink-100 p-4 text-sm">
                            ...
                            @endif
                            @if ($action['route'])
                                <a href="{{ $action['route'] }}" class="mt-3 inline-flex text-sm font-semibold text-mvhab-primary">Abrir ação</a>
                            @endif
                        </div>
                    @empty
                ...
                <div class="mt-4 divide-y divide-ink-100">
                    @forelse ($process->correctionRequests as $request)
                        <div class="flex flex-wrap items-center justify-between gap-3 py-4 text-sm">
                            ...
                            <a href="{{ route('candidate.correction-requests.show', $request) }}" class="font-semibold text-mvhab-primary">Consultar</a>
                        </div>
                    @empty
