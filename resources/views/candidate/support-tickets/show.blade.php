<x-app-layout>
    <x-slot name="header">
        <div>
            <p class="text-sm font-semibold text-mvhab-primary">Pedido {{ $ticket->ticket_number }}</p>
        ...
        <div class="mx-auto max-w-5xl space-y-6 px-4 sm:px-6 lg:px-8">
            ...
            @if ($ticket->acceptsCandidateReply())
                <form method="POST" action="{{ route('candidate.support-ticket-messages.store', $ticket) }}" class="mv-surface space-y-4 p-6">
                    ...
                    <textarea name="message" rows="5" class="w-full rounded-2xl border-ink-300 text-sm" required>{{ old('message') }}</textarea>
