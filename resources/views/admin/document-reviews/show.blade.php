<x-app-layout>
    <x-slot name="header">
        <div>
            <p class="text-sm font-semibold text-mvhab-primary">Revisão documental</p>
            <h1 class="mt-1 text-2xl font-semibold text-ink-900">{{ $submission->documentType->name }}</h1>
            <p class="mt-1 text-sm text-ink-500">
                {{ $submission->adhesionRegistration?->full_name ?? 'Candidato não indicado' }}
            </p>
        </div>
    </x-slot>
    ...
    <div class="py-8">
        <div class="mx-auto max-w-7xl space-y-6 px-4 sm:px-6 lg:px-8">
            ...
            <section class="grid gap-6 xl:grid-cols-[minmax(0,1fr)_24rem] xl:items-start">
                <div class="space-y-6">
                    <x-admin.document-preview :submission="$submission" />
                    ...
                    <div class="mv-surface p-6">
                        ...
                        @if ($submission->rejection_reason)
                            <div class="mt-5 rounded-2xl border border-red-100 bg-red-50 p-4">
                                <p class="text-sm font-semibold text-red-900">Motivo de rejeição visível ao candidato</p>
                                <p class="mt-1 text-sm leading-6 text-red-800">{{ $submission->rejection_reason }}</p>
                            </div>
                        @endif
                    </div>
                    ...
                    <div class="mv-surface p-6">
                        <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                            ...
                            @if ($latestAiAnalysis)
                                <a href="{{ route('backoffice.document-ai.assistant.show', $latestAiAnalysis) }}" class="mv-button-secondary">
                                    Ver assistente IA
                                </a>
                            @endif
                        </div>
                        ...
                        <dl class="mt-5 grid gap-4 sm:grid-cols-3">
                            <div>
                                ...
                                <dd class="mt-1 font-semibold text-ink-900">
                                    {{ $latestAiAnalysis?->status->label() ?? 'Sem análise executada' }}
                                </dd>
                            </div>
                            ...
                            <div>
                                ...
                                <dd class="mt-1 font-semibold text-ink-900">
                                    {{ $latestAiScore ? $latestAiScore->score.'%' : 'Sem score' }}
                                </dd>
                            </div>
                            ...
                            <div>
                                ...
                                <dd class="mt-1 font-semibold text-ink-900">
                                    {{ $latestAiScore?->requires_manual_review ? 'Recomendada' : 'Não indicada' }}
                                </dd>
                            </div>
                        </dl>
                    </div>
                ...
                <aside class="mv-surface p-6 xl:sticky xl:top-6">
                    ...
                    <div class="mt-4 grid gap-3">
                        <form method="POST" action="{{ route('admin.document-reviews.document-ai', $submission) }}">
                            ...
                            <p class="mt-2 text-xs leading-5 text-ink-500">
                                Gera score, flags e sugestões para apoio à revisão técnica.
                            </p>
                        </form>
                        ...
                        <form method="POST" action="{{ route('admin.document-reviews.validate', $submission) }}">
                            ...
                            <textarea
                                name="internal_notes"
                                rows="3"
                                class="mv-input mb-2 block w-full"
                        ...
                        <form method="POST" action="{{ route('admin.document-reviews.reject', $submission) }}" class="space-y-2">
                            ...
                            <textarea
                                ...
                                rows="3"
                                class="mv-input block w-full"
                                placeholder="Motivo visível ao candidato"
                                required
                            ></textarea>
                    ...
                    <div class="mt-5 border-t border-ink-100 pt-4">
                        <a href="{{ route('admin.document-reviews.download', $submission) }}" class="mv-button-secondary w-full">
                            Download seguro
                        </a>
                        <p class="mt-2 text-xs leading-5 text-ink-500">
                            Use apenas quando a pré-visualização não for suficiente.
                        </p>
                    </div>
                </aside>
            </section>
                        ...
                        @foreach ($submission->versions->sortByDesc('version_number') as $version)
                            <div class="py-3">
                                ...
                                <p class="text-sm text-ink-500">
                                    {{ $version->original_filename }} · {{ $version->status_at_upload->label() }}
                                </p>
                            </div>
                        @endforeach
                    </div>
                    ...
                    <div class="mt-4 divide-y divide-ink-100">
                        @forelse ($submission->reviews->sortByDesc('created_at') as $review)
                            <div class="py-3">
                                ...
                                <p class="text-sm text-ink-500">
                                    {{ $review->created_at->format('d/m/Y H:i') }} · {{ $review->reviewedBy?->name ?? 'Sistema' }}
                                </p>
